...
tasks.prototype.getAllTasksSince = async function (attributeName, value, condition = '==') {
  ...
  // experience and type. We assume the attribute is a number and we enforce
  // it for comparison. Other attributes may work ... or not. Test it.
  ...
  do {
    ...
    for (const task of pageTasks) {
      ...
      if ((condition == '==' && Number(task[attributeName]) == Number(value))
        || (condition == '>' && Number(task[attributeName]) > Number(value))
        || (condition == '>=' && Number(task[attributeName]) >= Number(value))
        || (condition == '<' && Number(task[attributeName]) < Number(value))
        || (condition == '<=' && Number(task[attributeName]) <= Number(value))) {
        tasks.push(task)
